      ...
      <el-table-column prop="name" label="文件名">
        <template slot-scope="scope">
          <div class="" style="cursor:pointer; text-align: left; text-indent: 15px;" @dblclick="gopath(scope.row.id, scope.row.name, scope.row.type)">
            <label>
              <!-- <input name="Fruit" type="checkbox" :checked="scope.row.btnRadio" :disabled="scope.row.transcode===2" :value="scope.row.id" @change="Changedropdown(scope.row)" /> -->
...
export default {
  ...
  data() {
    return {
      saasUploadFileUrl: window.localStorage.saasUploadFileUrl ? window.localStorage.saasUploadFileUrl : '',
      searchTxt: '',
