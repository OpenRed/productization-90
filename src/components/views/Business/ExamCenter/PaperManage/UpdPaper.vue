  ...
  <div style="width:100%;height:100%;">
    ...
    <el-form ref="form" :model="form" :rules="rules" label-width="100px" style="height:calc(100% - 120px);overflow-y:auto" id="form" method="post" >
      <el-form-item label="试卷名称" prop="name">
        <el-input v-model="form.name" style="width: 385px;" @change="" autofocus ></el-input>
      </el-form-item>
      <el-form-item label="考试时长" prop="duration">
        <el-input v-model="form.duration" style="width: 100px;" @change="" autofocus ></el-input> 分钟
      </el-form-item>
      ...
      <el-form-item label="及格线" style="margin-left: -14px" prop="passrate" v-if="confirmSubjectList.length!=0">
        ...
        <el-input v-model="form.passrate" style="width: 100px;" placeholder="范围 0-1.0" value="0" @change=""></el-input>
      ...
      <div id="TableDiv0" class="W95 Paper_List_Table">
      ...
      <div class="W95">题目指定分数</div>
      <br>
      <div id="TableDiv3" class="W95 Paper_List_Table" style="max-height:400px;min-height:150px;overflow-y:auto;">
      ...
      <el-form-item label="" style="margin-left:-80px">
        <el-button type="primary" style="width:120px" @click="OnUpdPaper">提交</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-dialog :visible.sync="dialogVisible" :title="titlename" top="5%" :before-close="closeDialog">
      <div style="width:100%; margin-top: -20px; padding: 8px 0;">
        <span>已选择 {{checkedSubject.filter(function(e){return e.status==10||e.status==12||e.status==11;}).length}} 题</span>
    ...
    <el-dialog :visible.sync="dialogVisible2" :title="titlename2" top="5%" @close="closeDialog2">
      ...
      <div style="width: 100%;height: 416px;overflow-x: hidden;border: 1px solid #cdcdcd;">
        <table border="1" bordercolor="#cdcdcd" style="width: 100%;text-align:center;border-collapse:collapse;">
          ...
          <tr v-for="item in checkedSubject.filter(function(e){return e.status==10||e.status==12||e.status==11;})" v-if="hackReset">
            <td>{{item.id}}</td>
            <td>{{item.content}}</td>
...
<script>
  import { getexample,getproblems,updPaper,getPaperById } from '@/api/api';
  ...
  export default {
    data () {
      ...
      return {
        ...
        Gloading: false,
        NotDis:false,
        title: window.localStorage.getItem("saasaddPaper"),
        dialogVisible: false,
    ...
    mounted(){
      ...
      if(this.title==='编辑试卷'){
        getPaperById(window.localStorage.getItem('saasPaperid')).then(data=>{
          // console.log(data);
          if(data.success) {
            this.form.name = data.name;
            this.form.duration = data.duration;
            this.form.passrate = data.passrate;
            this.totalScore = data.total;
            this.checkedSubject = data.plist;
            // item.status=10，题目状态为原有
            this.checkedSubject.forEach(item=>{ item.status = 10; });
            this.confirmSubjectList = data.plist;
            var index = 0;
            this.confirmSubjectList.forEach(item=>{
              if(item.type==1){ item.typeName = '单选题'; }
              if(item.type==2){ item.typeName = '判断题'; }
            ...
            // console.log('================');
            // console.log(data.gspList);
            data.gspList.forEach(item2=>{
              if(item2.model==1) this.subjectType1 = item2.avg||0;
              if(item2.model==2) this.subjectType2 = item2.avg||0;
              if(item2.model==3) this.subjectType3 = item2.avg||0;
              if(item2.model==4) this.subjectType4 = item2.avg||0;
              if(item2.model==5) this.subjectType5 = item2.avg||0;
            });
          }
        });
        this.setDisabled();
    ...
    updated(){
      this.confirmSubjectList.forEach(item=>{
        document.getElementById(item.id.toString()).value = item.score;
      });
    },
    computed:{
      totalType1(){ return this.checkedSubject.filter(function(e){return e.type==1;}).length * this.subjectType1 || 0; },
      totalType2(){ return this.checkedSubject.filter(function(e){return e.type==2;}).length * this.subjectType2 || 0; },
      totalType3(){ return this.checkedSubject.filter(function(e){return e.type==3;}).length * this.subjectType3 || 0; },
      totalType4(){ return this.checkedSubject.filter(function(e){return e.type==4;}).length * this.subjectType4 || 0; },
      totalType5(){ return this.checkedSubject.filter(function(e){return e.type==4;}).length * this.subjectType5 || 0; },
    },
    methods:{
      openSuccess(text) {
        this.$message({
      ...
      OnUpdPaper(){
        this.plist = [];
        this.form.id=window.localStorage.getItem('saasPaperid');
        ...
        updPaper(this.form, this.plist).then(data=>{
          if(data.success){
              ...
              this.subjectList.forEach(item=>{
                ...
                if(this.checkedSubject!=''){
                  this.checkedSubject.forEach(i=>{
                    if(i.id==item.id){
                      if(i.status==10||i.status==11||i.status==12) item.isCheck = true;
                    }
                  });
                }
              });
      ...
      up(item){
        // console.log(item.index, this.checkedSubject.indexOf(item));
      ...
      confirmShow(){
        for(var i=0; i<this.checkedSubject.length; i++){
          ...
          if(item.status==100||item.status==110||item.status==120||item.status==130) {
